...
*Setup instructions for a gitian build of SLTC using a Debian VM or physical system.*
...
- [Building SLTC](#building-slatechain)
...
Clone the git repositories for slatechain and gitian and then checkout the slatechain version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/slatechain-crypto/slatechain.git
cd slatechain
...
Follow the instructions in [doc/release-process.md](release-process.md) in the slatechain repository
...
To build SLTC (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the slatechain repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/slatechain/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/slatechain-crypto/slatechain
...
For example:
```bash
URL=https://github.com/crowning-/slatechain.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit slatechain=${COMMIT} --url slatechain=${URL} ../slatechain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit slatechain=${COMMIT} --url slatechain=${URL} ../slatechain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit slatechain=${COMMIT} --url slatechain=${URL} ../slatechain/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/slatechain-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/slatechain-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/slatechain-build.assert
```
...
[slatechain/gitian.sigs](https://github.com/slatechain-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
